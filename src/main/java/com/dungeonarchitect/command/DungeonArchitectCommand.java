...
public final class DungeonArchitectCommand implements CommandExecutor, TabCompleter {
    ...
    private void doorCommand(CommandSender sender, String[] args) {
        ...
        switch (args[1].toLowerCase(Locale.ROOT)) {
            case "create" -> {
                requireArgs(args, 3, "/da door create <id>");
                String doorId = args[2];
                player.sendMessage(Component.text("Preparing isolated edit workspace for door " + doorId + "..."));
                authoringManager.createDoorSession(player, doorId).whenComplete((session, error) -> {
                    if (error != null) {
                        player.sendMessage(Component.text("Failed to start door edit: " + message(error)));
                        return;
                    }
                    player.sendMessage(Component.text("Authoring door " + doorId + " in da_edit. Select bounds, then /da door bounds."));
                });
            }
            case "bounds" -> {
                var bounds = authoringManager.saveCurrentSelectionAsRoomBounds(player);
            ...
            case "marker" -> {
                ...
                var local = authoringManager.targetedLocalPosition(player)
                    .orElseThrow(() -> new IllegalArgumentException("Look at a block inside selected bounds"));
                String type = args.length >= 5 ? args[4] : "generic";
                authoringManager.addDoorMarker(player, args[3], type, local);
                player.sendMessage(Component.text("Added door marker " + args[3] + " at " + local + "."));
            }
            case "feature" -> {
                String slotName = args.length >= 3 ? args[2] : null;
                var slot = authoringManager.createDoorFeatureSlotFromSelection(player, slotName);
                player.sendMessage(Component.text("Added door feature slot " + slot.id() + " at " + slot.position() + " size=" + slot.size() + "."));
            }
            case "save" -> {
                String id = args.length >= 3 ? args[2] : null;
                try {
                    TemplateValidationResult result = authoringManager.saveDoor(player, id);
                    player.sendMessage(Component.text("Door saved."));
                    sendValidation(player, result);
                    doorRegistry.reload();
                    templateRegistry.reload();
                } catch (Exception ex) {
                    throw new IllegalArgumentException("Failed to save door: " + ex.getMessage(), ex);
                }
            }
            case "edit" -> {
                ...
                authoringManager.editDoorSession(player, template).whenComplete((session, error) -> {
                    if (error != null) {
                        player.sendMessage(Component.text("Failed to paste door for editing: " + message(error)));
                        return;
                    }
                    player.sendMessage(Component.text("Pasted door " + template.id() + " into the edit world. Save with /da door save."));
                });
            }
    ...
    private void inspectDoor(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(Component.text("metadata size=" + template.size() + " tags=" + template.tags() + " structure=" + template.structureFile()));
        sender.sendMessage(Component.text("gateway=" + (template.gateway() == null ? "unset" : template.gateway())));
        sender.sendMessage(Component.text("markers=" + template.markers().size() + " featureSlots=" + template.featureSlots().size()));
        try {
            sender.sendMessage(Component.text("door.nbt size=" + structureService.loadSize(template.structureFile())));
        } catch (Exception ex) {
            sender.sendMessage(Component.text("door.nbt size=ERROR " + ex.getMessage()));
        }
    }

    private void validateDoor(CommandSender sender, String[] args) {
        requireArgs(args, 3, "/da door validate <id|all>");
        if (args[2].equalsIgnoreCase("all")) {
    ...
    private void deleteDoor(CommandSender sender, String[] args) {
